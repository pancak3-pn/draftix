-- Multi-format tournaments: add double elimination, round robin, and Swiss.
...
alter table public.tournament_matches add constraint tournament_matches_round_number_check
  check (round_number between 1 and 15);
alter table public.tournament_matches add column if not exists next_match_loser_id uuid references public.tournament_matches(id) on delete set null;
alter table public.tournament_matches add column if not exists next_slot_loser text check (next_slot_loser in ('A','B'));
-- Marks an empty double-elimination losers match whose feeders were both byes.
alter table public.tournament_matches add column if not exists is_empty boolean not null default false;

-- Live series scoring: teams that cannot be decided in a single map
-- (best of 3 / best of 5) can record a live running score that the public
-- sees immediately, without advancing the bracket. The bracket only
-- advances when `draftix_set_match_result` records a winner who reached the
-- required number of wins.

create or replace function public.draftix_update_series_score(p_slug text, p_token text, p_match_id uuid, p_score_a integer, p_score_b integer)
...
begin
  ...
  if m.winner_team_id is not null then raise exception 'Match is already decided'; end if;
  ...
  if p_score_a is null and p_score_b is null then
    update public.tournament_matches set score_a=null,score_b=null,updated_at=now() where id=m.id;
  else
    if p_score_a is null or p_score_b is null or p_score_a<0 or p_score_b<0 or p_score_a>99 or p_score_b>99
       or p_score_a+p_score_b > t.best_of then
      raise exception 'Enter a valid live score for best of %', t.best_of;
    end if;
    update public.tournament_matches set score_a=p_score_a,score_b=p_score_b,updated_at=now() where id=m.id;
  end if;
  update public.tournaments set updated_at=now() where id=t.id;
  return jsonb_build_object('ok',true);
end $$;

grant execute on function public.draftix_update_series_score(text,text,uuid,integer,integer) to authenticated;
...
begin
  ...
  if greatest(p_score_a,p_score_b) <> ceil(t.best_of/2.0) or p_score_a+p_score_b > t.best_of then
    raise exception 'Enter the final series score: the winner needs % wins in best of %', ceil(t.best_of/2.0), t.best_of;
  end if;

  if t.format in ('single_elimination','double_elimination') then
    ...
    if t.format='double_elimination' and m.next_match_loser_id is not null then
      select * into downstream from public.tournament_matches where id=m.next_match_loser_id for update;
      if downstream.winner_team_id is not null then
        raise exception 'Clear the next match before changing this winner';
      end if;
      update public.tournament_matches set
        team_a_id=case when m.next_slot_loser='A' then (case when p_winner_team_id=m.team_a_id then m.team_b_id else m.team_a_id end) else team_a_id end,
        team_b_id=case when m.next_slot_loser='B' then (case when p_winner_team_id=m.team_a_id then m.team_b_id else m.team_a_id end) else team_b_id end,
        updated_at=now()
      where id=m.next_match_loser_id;
      -- Auto-advance a bye if the receiving match now has one participant and
      -- its other feeder was an empty match.
      loop
        select * into downstream from public.tournament_matches where id=m.next_match_loser_id for update;
        exit when downstream.is_empty or downstream.winner_team_id is not null
          or (downstream.team_a_id is not null) = (downstream.team_b_id is not null)
          or not exists(select 1 from public.tournament_matches f where (f.next_match_id=downstream.id or f.next_match_loser_id=downstream.id) and f.is_empty);
        update public.tournament_matches set
          winner_team_id=coalesce(downstream.team_a_id,downstream.team_b_id),updated_at=now()
        where id=downstream.id;
        exit when downstream.next_match_id is null;
        update public.tournament_matches set
          team_a_id=case when downstream.next_slot='A' then coalesce(downstream.team_a_id,downstream.team_b_id) else team_a_id end,
          team_b_id=case when downstream.next_slot='B' then coalesce(downstream.team_a_id,downstream.team_b_id) else team_b_id end,
          updated_at=now()
        where id=downstream.next_match_id;
        m.next_match_loser_id := downstream.next_match_id;
      end loop;
    end if;
  end if;
  ...
  elsif t.format='round_robin' then
    update public.tournaments set status=case when exists(select 1 from public.tournament_matches where tournament_id=t.id and winner_team_id is null) then 'live' else 'completed' end,updated_at=now() where id=t.id;
  else
    if not exists(select 1 from public.tournament_matches where tournament_id=t.id and round_number=m.round_number and winner_team_id is null)
       and m.round_number < t.total_rounds then
      perform public.draftix_swiss_pair_next_round(t.id, m.round_number);
    end if;
    update public.tournaments set status=case
      when exists(select 1 from public.tournament_matches where tournament_id=t.id and winner_team_id is null) then 'live'
      when m.round_number >= t.total_rounds then 'completed'
      else 'live' end,updated_at=now() where id=t.id;
  end if;
  return jsonb_build_object('ok',true);
end $$;

create or replace function public.draftix_swiss_pair_next_round(p_tournament_id uuid, p_finished_round integer)
...
begin
  ...
  if not found or t.format <> 'swiss' then raise exception 'Tournament not found'; end if;
  if p_finished_round >= t.total_rounds then return; end if;
  next_round := p_finished_round+1;
  create temp table swiss_queue on commit drop as
    select row_number() over (order by s.wins desc,s.diff desc,s.seed asc) ord,s.team_id,false taken
    from (
      select tm.id team_id,tm.seed,
        count(m2.id) filter (where m2.winner_team_id=tm.id) wins,
        coalesce(sum(case when m2.winner_team_id=tm.id then coalesce(abs(m2.score_a-m2.score_b),0) else -coalesce(abs(m2.score_a-m2.score_b),0) end),0) diff
      from public.tournament_teams tm
      left join public.tournament_matches m2 on m2.tournament_id=tm.tournament_id and m2.winner_team_id is not null and tm.id in (m2.team_a_id,m2.team_b_id)
      where tm.tournament_id=t.id
      group by tm.id,tm.seed
    ) s;
  pair_pos := 0;
  loop
    select into cur ord,team_id from swiss_queue where not taken order by ord limit 1;
    exit when cur.team_id is null;
    update swiss_queue set taken=true where ord=cur.ord;
    select into opp q.ord,q.team_id from swiss_queue q where not q.taken
      order by exists(select 1 from public.tournament_matches x where x.tournament_id=t.id and x.round_number<=p_finished_round
          and ((x.team_a_id=cur.team_id and x.team_b_id=q.team_id) or (x.team_a_id=q.team_id and x.team_b_id=cur.team_id))),
        q.ord
      limit 1;
    pair_pos := pair_pos+1;
    if opp.team_id is null then
      insert into public.tournament_matches(tournament_id,round_number,position,team_a_id,winner_team_id)
        values(t.id,next_round,pair_pos,cur.team_id,cur.team_id);
    else
      update swiss_queue set taken=true where ord=opp.ord;
      insert into public.tournament_matches(tournament_id,round_number,position,team_a_id,team_b_id)
        values(t.id,next_round,pair_pos,cur.team_id,opp.team_id);
    end if;
  end loop;
end $$;
...
begin
  ...
  if t.format='swiss' and exists(select 1 from public.tournament_matches where tournament_id=t.id and round_number>m.round_number) then
    raise exception 'Clear later Swiss rounds before changing this result';
  end if;
  if t.format in ('single_elimination','double_elimination') and m.next_match_id is not null then
    select * into downstream from public.tournament_matches where id=m.next_match_id for update;
  ...
  if t.format='double_elimination' and m.next_match_loser_id is not null then
    ...
    if downstream.winner_team_id is not null then raise exception 'Clear the next match first'; end if;
    update public.tournament_matches set
      team_a_id=case when m.next_slot_loser='A' then null else team_a_id end,
      team_b_id=case when m.next_slot_loser='B' then null else team_b_id end,
      updated_at=now()
    where id=m.next_match_loser_id;
  end if;
...
begin
  ...
  return jsonb_build_object(
    ...
    'bestOf',t.best_of,'status',t.status,'createdAt',t.created_at,'totalRounds',t.total_rounds,
    ...
    'championTeamId',case
      when t.format in ('single_elimination','double_elimination') then
        (select m3.winner_team_id from public.tournament_matches m3 where m3.tournament_id=t.id and m3.next_match_id is null and m3.winner_team_id is not null limit 1)
      when t.status='completed' then
        (select s.team_id from (
          select tm.id team_id,tm.seed,
            count(m2.id) filter (where m2.winner_team_id=tm.id) wins,
            coalesce(sum(case when m2.winner_team_id=tm.id then coalesce(abs(m2.score_a-m2.score_b),0) else -coalesce(abs(m2.score_a-m2.score_b),0) end),0) diff
          from public.tournament_teams tm
          ...
          where tm.tournament_id=t.id
          group by tm.id,tm.seed
          order by wins desc,diff desc,tm.seed limit 1
        ) s)
      else null end,
    'standings',coalesce((
      select jsonb_agg(jsonb_build_object('teamId',s.team_id,'name',s.name,'seed',s.seed,'wins',s.wins,'losses',s.losses,'diff',s.diff,'played',s.played) order by s.wins desc,s.diff desc,s.seed)
      from (
        select tm.id team_id,tm.name,tm.seed,
          count(m2.id) filter (where m2.winner_team_id=tm.id) wins,
          count(m2.id) filter (where m2.winner_team_id is not null and m2.winner_team_id<>tm.id) losses,
          coalesce(sum(case when m2.winner_team_id=tm.id then coalesce(abs(m2.score_a-m2.score_b),0) else -coalesce(abs(m2.score_a-m2.score_b),0) end),0) diff,
          count(m2.id) filter (where m2.winner_team_id is not null) played
        from public.tournament_teams tm
        left join public.tournament_matches m2 on m2.tournament_id=tm.tournament_id and m2.winner_team_id is not null and tm.id in (m2.team_a_id,m2.team_b_id)
        where tm.tournament_id=t.id
        group by tm.id,tm.name,tm.seed
      ) s),'[]'::jsonb)
  );
end $$;

revoke all on function public.draftix_swiss_pair_next_round(uuid,integer) from public,anon,authenticated;
